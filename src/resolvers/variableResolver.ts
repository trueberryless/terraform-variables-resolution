...
export class TerraformVariableResolver {
  ...
  constructor(
    ...
    this.logger.info(
      `TerraformVariableResolver initialized for: ${workspaceRoot}`
    );
  }
  ...
  async resolveVariableValue(
    ...
    if (depth > this.maxRecursionDepth) {
      this.logger.warn(
        `Maximum recursion depth reached for variable: ${variableName}`
      );
      return null;
    }
    ...
    if (visited.has(visitKey)) {
      this.logger.debug(
        `Circular reference detected for: ${variableName} in ${currentDir}`
      );
      return null;
    }
    ...
    try {
      ...
      if (variableName.startsWith("module.")) {
        resolvedValue = await this.resolveModuleOutput(
          variableName,
          currentDir,
          visited,
          depth + 1
      ...
      if (parentDir !== currentDir && this.isWithinWorkspace(parentDir)) {
        resolvedValue = await this.resolveVariableValue(
          variableName,
          parentDir,
          visited,
          depth + 1
      ...
      this.logger.error(
        `Error resolving variable ${variableName} in ${currentDir}`,
        error
      );
      return null;
    } finally {
  ...
  private async findInTfvarsFiles(
    variableName: string,
    dir: string
  ): Promise<string | null> {
    try {
      ...
      const tfvarsFiles = files.filter(
        (f) => f.endsWith(".tfvars") || f.endsWith(".tfvars.json")
      );
  ...
  async resolveVariableValueEnhanced(
    ...
    try {
      ...
      resolvedValue = await this.findInTfvarsFiles(variableName, currentDir);
      if (resolvedValue !== null) {
        const finalValue = await this.resolveNestedReferences(
          resolvedValue,
          currentDir,
          visited,
          depth + 1
      ...
      if (moduleContext.length === 0) {
        const moduleVariable = await this.resolveAsModuleInput(
          variableName,
          currentDir,
          visited,
          depth + 1
      ...
      if (parentDir !== currentDir && this.isWithinWorkspace(parentDir)) {
        ...
        const parentModuleValue = await this.findVariableInParentModule(
          variableName,
          currentDir,
          parentDir,
          visited,
        ...
        resolvedValue = await this.resolveVariableValueEnhanced(
          variableName,
          parentDir,
          visited,
          depth + 1,
      ...
      this.logger.error(
        `Error in enhanced resolution for ${variableName} in ${currentDir}`,
        error
      );
      return null;
    } finally {
  ...
  private async resolveAsModuleInput(
    ...
    try {
      ...
      const moduleCalls = await this.findModuleCallsToDirectory(
        parentDir,
        moduleDir
      );
      ...
      for (const moduleCall of moduleCalls) {
        ...
        if (moduleCall.variables[variableName]) {
          ...
          const resolvedRef = await this.resolveVariableReference(
            variableReference,
            parentDir,
            visited,
            depth
          ...
          if (resolvedRef !== null) {
            this.logger.debug(
              `Resolved ${variableName} via module input: ${variableReference} -> ${resolvedRef}`
            );
            return resolvedRef;
          }
        }
      ...
      this.logger.error(
        `Error resolving module input for ${variableName}`,
        error
      );
      return null;
    }
  ...
  private async findModuleCallsToDirectory(
    searchDir: string,
    targetDir: string
  ): Promise<
    Array<{
    ...
    try {
      ...
      for (const file of tfFiles) {
        ...
        while ((match = moduleRegex.exec(content)) !== null) {
          ...
          const resolvedSource = await this.resolveModulePath(
            sourceMatch[1],
            searchDir
          );
  ...
  private async resolveVariableReference(
    ...
    try {
      ...
      if (cleanRef.startsWith("var.")) {
        ...
        return await this.resolveVariableValueEnhanced(
          varName,
          contextDir,
          visited,
          depth
        ...
        return await this.resolveModuleOutput(
          cleanRef,
          contextDir,
          visited,
          depth
      ...
      this.logger.error(
        `Error resolving variable reference: ${reference}`,
        error
      );
      return null;
    }
    ...
    try {
      ...
      for (const moduleCall of moduleCalls) {
        ...
        if (moduleCall.variables[variableName]) {
          ...
          const resolved = await this.resolveVariableReference(
            variableRef,
            parentDir,
            visited,
            depth
  ...
  private async resolveModuleOutput(
    ...
    try {
      ...
      if (!moduleCall) {
        this.logger.debug(
          `Module call not found: ${moduleName} in ${currentDir}`
        );
        return null;
      }
      ...
      if (!modulePath) {
        this.logger.debug(
          `Module path could not be resolved: ${moduleCall.source}`
        );
        return null;
      }
      ...
      if (outputValue !== null) {
        ...
        return await this.resolveReferencesInValue(
          outputValue,
          modulePath,
          visited,
          depth + 1
  ...
  private async resolveReferencesInValue(
    ...
    try {
      ...
      for (const varRef of varRefs) {
        const resolvedRef = await this.resolveVariableValue(
          varRef,
          contextDir,
          visited,
          depth
  ...
  private async findModuleCall(
    moduleName: string,
    dir: string
  ): Promise<ModuleCall | null> {
    try {
      ...
      this.logger.error(
        `Error finding module call: ${moduleName} in ${dir}`,
        error
      );
      return null;
    }
  ...
  private async resolveModulePath(
    ...
    try {
      ...
      // Handle registry modules (not supported for local resolution)
        ...
        this.logger.debug(
          `Registry module not supported for local resolution: ${source}`
        );
        return null;
      }
