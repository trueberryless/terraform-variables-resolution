...
export class TerraformParser {
    ...
    parseHclVariable(content: string, variableName: string): string | null {
        try {
            ...
            const patterns = [
                ...
                new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*"([^"]*)"\\s*$`, 'm'),
                
                // Unquoted value: variable_name = value
                new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*([^\\s\\n#\\{\\[]+)\\s*(?:#.*)?$`, 'm'),
                
                // Array assignment: variable_name = ["value1", "value2"]
                new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*(\\[[^\\]]*\\])\\s*$`, 'm'),
                
                // FIXED: Multi-line object assignment with proper brace matching
                new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*(\\{[\\s\\S]*?\\})\\s*(?:\\n|$)`, 'gm'),
                
                // Boolean values
                new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*(true|false)\\s*$`, 'm'),
                
                // Numeric values
                new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*(\\d+(?:\\.\\d+)?)\\s*$`, 'm'),
                
                // Heredoc strings
                new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*<<-?\\s*(\\w+)\\s*\\n([\\s\\S]*?)^\\s*\\1\\s*$`, 'gm')
            ];
    ...
    private parseComplexObject(objectValue: string, fullContent: string, variableName: string): string {
        try {
            ...
            this.logger.error(`Error parsing complex object for ${variableName}`, error);
    ...
    private extractCompleteObject(content: string, variableName: string): string {
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (!inObject && line.match(new RegExp(`^\\s*${this.escapeRegex(variableName)}\\s*=\\s*\\{`))) {
                    inObject = true;
                    objectStart = i;
            ...
            this.logger.error(`Error extracting complete object for ${variableName}`, error);
            return '{}';
        }
    }
    ...
    private convertHclObjectToJson(hclObj: string): string {
        ...
        jsonStr = jsonStr
            ...
            .replace(/:\s*([^",\\{\\[\\n\\r]+)(\s*[,\\}\\n\\r])/g, (match, value, suffix) => {
                const trimmedValue = value.trim();
                if (trimmedValue === 'true' || trimmedValue === 'false' || /^\\d+(\\.\\d+)?$/.test(trimmedValue)) {
    ...
    parseLocalsBlock(content: string, variableName: string): string | null {
        try {
            ...
            this.logger.error(`Error parsing locals block for ${variableName}`, error);
            return null;
        }
    }
    ...
    parseOutputBlock(content: string, outputName: string): string | null {
        try {
            ...
            if (match) {
                ...
                const multiLineValueMatch = /value\s*=\s*\{([^}]*(?:\{[^}]*\}[^}]*)*)\}/gs.exec(outputContent);
    ...
    parseModuleVariables(moduleContent: string): { [key: string]: string } {
        ...
        try {
            ...
            for (const line of lines) {
                ...
                if (trimmedLine.startsWith('source') || trimmedLine.startsWith('#') || !trimmedLine.includes('=')) {
                    continue;
                }
    ...
    private formatValue(value: any): string {
        ...
        if (Array.isArray(value)) {
            ...
            if (value.length > 3) {
                return `[${value.slice(0, 3).map(v => this.formatValue(v)).join(', ')}, ...]`;
            }
