...
export class TerraformInlayProvider implements vscode.InlayHintsProvider {
    ...
    constructor(
        ...
        this.logger.info(`TerraformInlayProvider initialized for workspace: ${workspaceRoot}`);
    }
    ...
    async provideInlayHints(
        ...
        const stopwatch = this.performanceMonitor.startTimer(`provideInlayHints-${path.basename(document.fileName)}`);
        
        try {
            const hints = await this.generateInlayHints(document, range, token);
            stopwatch.stop();
            
            this.logger.debug(`Generated ${hints.length} hints for ${document.fileName} in ${stopwatch.getDuration()}ms`);
            return hints;
        } catch (error) {
            stopwatch.stop();
            this.logger.error(`Failed to provide inlay hints for ${document.fileName}`, error);
            return [];
        }
    }
        ...
        for (const pattern of patterns) {
            ...
            while ((match = pattern.regex.exec(text)) !== null) {
                ...
                try {
                    ...
                    if (pattern.type === 'variable_property' || pattern.type === 'local_property') {
                    ...
                    const allResolvedValues = await this.resolveVariableAllContextsWithProperty(
                        variableName,
                        propertyName,
                        currentDir,
                        pattern.type
                    ...
                    this.logger.error(`Error processing variable match: ${match[0]}`, error);
    ...
    private async resolveVariableAllContextsWithProperty(
        ...
        const resolvedValues: Array<{value: string, context: string, source: string}> = [];
        ...
        const commonEnvPaths = [
            'environments/dev', 'environments/test', 'environments/production', 'environments/staging',
            'env/dev', 'env/test', 'env/production', 'env/staging',
        ...
        for (const searchPath of searchPaths) {
            try {
                ...
                if (propertyName) {
                    ...
                    resolvedValue = await this.resolveObjectProperty(variableName, propertyName, searchPath);
                } else {
                    // Regular variable resolution with enhanced recursion
                    resolvedValue = await this.resolver.resolveVariableValueEnhanced(variableName, searchPath);
                }
                
                if (resolvedValue && resolvedValue.trim() !== '') {
                    const contextName = this.getContextName(searchPath, workspaceRoot);
                    const existingValue = resolvedValues.find(rv => rv.value === resolvedValue);
                ...
                this.logger.debug(`Could not resolve ${variableName}${propertyName ? '.' + propertyName : ''} in ${searchPath}`);
            }
        }
    ...
    private async resolveObjectProperty(
        ...
        try {
            ...
            const objectValue = await this.resolver.resolveVariableValueEnhanced(objectName, searchPath);
            ...
            this.logger.error(`Error resolving object property ${objectName}.${propertyName}`, error);
            return null;
        }
    }

    // NEW: Extract property from resolved object
    private extractPropertyFromObject(objectValue: string, propertyName: string): string | null {
        try {
            ...
            const propertyRegex = new RegExp(`${this.escapeRegex(propertyName)}\\s*=\\s*"([^"]*)"`, 'i');
            ...
            // Unquoted values
            const unquotedRegex = new RegExp(`${this.escapeRegex(propertyName)}\\s*=\\s*([^\\s,}]+)`, 'i');
            ...
            this.logger.error(`Error extracting property ${propertyName} from object`, error);
            return null;
        }
    }
    ...
    private async resolveVariableWithCache(
        ...
        try {
            ...
            this.cacheMisses++;
            const resolved = await this.resolver.resolveVariableValue(variableName, currentDir);
    ...
    private formatResolvedValue(value: string): string {
        ...
        if (!value.startsWith('"') && !value.startsWith("'") && 
            !value.match(/^[\d.]+$/) && !value.match(/^(true|false)$/)) {
            return `"${value}"`;
        }
    ...
    private createInlayHintWithMultipleValues(
        ...
        if (resolvedValues.length === 1) {
            ...
            if (this.isComplexObject(singleValue.value)) {
                ...
                tooltipContent = this.createComplexObjectTooltip(variableName, singleValue.value, singleValue.context);
            } else {
                displayText = ` → ${formattedValue}`;
                tooltipContent = this.createSingleValueTooltip(variableName, formattedValue, singleValue.context);
            }
        } else {
            // Multiple values
    ...
    private createComplexObjectTooltip(variableName: string, value: string, context: string): string {
        return `**Terraform Variable:** \`${variableName}\`\n\n` +
            `**Context:** \`${context}\`\n\n` +
    ...
    private createSingleValueTooltip(variableName: string, value: string, context: string): string {
        return `**Terraform Variable:** \`${variableName}\`\n\n` +
            `**Context:** \`${context}\`\n\n` +
    ...
    private createJsonFromResolvedValues(
        ...
        for (const resolved of resolvedValues) {
            try {
                ...
                if ((resolved.value.trim().startsWith('{') && resolved.value.trim().endsWith('}')) ||
                    (resolved.value.trim().startsWith('[') && resolved.value.trim().endsWith(']'))) {
                    try {
    ...
    private isInStringOrComment(document: vscode.TextDocument, position: vscode.Position): boolean {
        try {
            ...
            this.logger.error('Error checking if position is in string or comment', error);
            return false;
        }
    }
}