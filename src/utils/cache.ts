...
export class TerraformCache {
    ...
    invalidateFile(filePath: string): void {
        try {
            ...
            if (deletedCount > 0) {
                this.logger.debug(`Invalidated ${deletedCount} cache entries for file: ${filePath}`);
            }
        } catch (error) {
            this.logger.error(`Error invalidating cache for file: ${filePath}`, error);
        }
    }
    ...
    getStats(): { size: number; hitRate: number; oldestEntry: number; newestEntry: number } {
        ...
        const avgAccessPerEntry = this.cache.size > 0 ? totalAccess / this.cache.size : 0;
