...
export function activate(context: vscode.ExtensionContext) {
    ...
    if (!workspaceFolders || workspaceFolders.length === 0) {
        ...
        vscode.window.showWarningMessage('Terraform Variable Resolver: No workspace folder found');
        return;
    }
    ...
    for (const folder of workspaceFolders) {
        try {
            ...
            const provider = new TerraformInlayProvider(workspaceRoot, logger, configManager);
            inlayProviders.set(workspaceRoot, provider);

            // Register inlay hints provider
            const inlayProviderDisposable = vscode.languages.registerInlayHintsProvider(
                { scheme: 'file', language: 'terraform', pattern: `${workspaceRoot}/**/*.tf` },
                provider
            );
            ...
            logger.error(`Failed to initialize provider for workspace ${folder.name}`, error);
            vscode.window.showErrorMessage(`Terraform Variable Resolver: Failed to initialize for workspace ${folder.name}`);
        }
    }
    ...
    vscode.window.showInformationMessage('Terraform Variable Resolver activated!');
}
    ...
    const commands = [
        vscode.commands.registerCommand('terraform-resolver.refresh', async () => {
            try {
                ...
                vscode.window.showInformationMessage('Terraform variable cache refreshed successfully');
        ...
        vscode.commands.registerCommand('terraform-resolver.toggle', async () => {
            try {
                ...
                vscode.window.showErrorMessage('Failed to toggle Terraform inlay hints');
            }
        }),

        vscode.commands.registerCommand('terraform-resolver.clearCache', async () => {
            try {
                for (const provider of inlayProviders.values()) {
                    provider.clearCache();
                }
                vscode.window.showInformationMessage('Terraform cache cleared successfully');
        ...
        vscode.commands.registerCommand('terraform-resolver.diagnostics', async () => {
            try {
...
function handleWorkspaceFolderChanges(
    ...
    for (const folder of event.added) {
        try {
            ...
            logger.error(`Failed to initialize provider for new workspace ${folder.name}`, error);
            vscode.window.showErrorMessage(`Failed to initialize Terraform resolver for workspace ${folder.name}`);
        }
    }
}
...
export async function deactivate(): Promise<void> {
    ...
    try {
        ...
        const disposePromises = Array.from(inlayProviders.values()).map(async provider => {
            try {
